...
angular
    ...
    .run(['$ionicPlatform','$rootScope','$timeout',function($ionicPlatform, $rootScope, $timeout) {
...
//Controller Principal
    .controller('mainwindowController', ['$scope', 'juronsFactory', function($scope, juronsFactory) {
        ...
        $scope.captainInsult = function() {
            ...
            setTimeout(function() {
                $('#captainspeaking').removeClass('animated bounce')
            }, 1000);
        };
...
//Controller Notification locale
    .controller("NotificationController", ['$scope', 'juronsFactory', '$cordovaLocalNotification', '$q', '$rootScope', function($scope, juronsFactory, $cordovaLocalNotification, $q, $rootScope) {
        //Initialis tion de la bd
        $scope.db = new PouchDB('captaineh');

        //Récupération des données et traitement de l'état de notification
        $scope.db.get('uniqrow').then(function(doc) {
            $scope.activation = doc.state;
            ...
            $scope.db.put({
                ...
                // handle response
                console.log('Insertion réussie après détection vide');
            }).catch(function(err) {
                console.log(err);
            });
        });

        //Mise à jour de la notification à true
        $scope.updateTotruestatenotification = function() {
            var defered =$q.defer();
            $scope.db.get('uniqrow').then(function(doc) {
                $scope.db.put({
                    state: true
        ...
        //Mise à jour de la notification à false
        $scope.updateTofalsestatenotification = function() {
            var defered =$q.defer();
            $scope.db.get('uniqrow').then(function(doc) {
                $scope.db.put({
                    state: false
        ...
        $scope.bordee = function() {
            var newjurons = juronsFactory.injures;
            var newbordee = "";
        ...
        $scope.add = function() {
            var nextreize = new Date().setHours(13, 00, 00);
            ...
            if (now <= treizeam) {
                for(var i=0; i<365; i++){
                    var timeup = treizeam.getTime() + i*24*60*60*1000;
                    timeup = new Date(timeup);
                    tabobj[i] = {
                        id: i,
                ...
                $cordovaLocalNotification.schedule(tabobj).then(function() {
                    console.log("The notification has been set");
                    $scope.updateTotruestatenotification();
                });
                //Message de confirmation d'activation
        ...
        $scope.desactiver = function() {
            $cordovaLocalNotification.cancelAll().then(function(cancelAll) {
                $scope.updateTofalsestatenotification();
            });
